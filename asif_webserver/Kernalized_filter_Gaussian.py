...
def preprocess(I):
    ...
    Z = np.zeros([I.shape[0],I.shape[1],1])
    Z[:,:,0] = rgb2hed(I)[:,:,0]                           #Hematoxyline Features  
    Z[:,:,0] = (Z[:,:,0]/255.0)**2
    Z[:,:,0] = meanunit(Z[:,:,0])
...
class Classifier:
    ...
    def TestResponse(self):         #return the response immage to the test image  in spatial domain
          ...
          return self.RemovePadding (response)                                     #result is in spatial domain
